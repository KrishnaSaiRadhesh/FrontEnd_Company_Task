...
const Comments = () => {
  ...
  const calculateResponsiveConfig = () => {
    if (typeof window === "undefined")
      return { numVisible: 3, cardWidth: 336, gap: 32 };
  ...
  const maxIndex = Math.max(
    0,
    testimonials.length - responsiveConfig.numVisible
  );
  ...
  const getContainerWidth = () => {
    ...
    if (width < 1024) {
      return (
        responsiveConfig.numVisible * responsiveConfig.cardWidth +
  ...
  const containerWidth =
    typeof window !== "undefined" ? getContainerWidth() : 1200;
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const newMaxIndex = Math.max(
        0,
        testimonials.length - newConfig.numVisible
      );
